...
public class PostDAO {
    ...
    public List<PostBean> getByDateWithPagination(int page, int limit) throws SQLException {
        Connection con = null;
        PreparedStatement statement = null;
        List<PostBean> posts = new ArrayList<>();
        ...
        // SQL con LIMIT e OFFSET per la paginazione
        String query = "SELECT * FROM " + TABLE_NAME + " ORDER BY dataCreazione DESC LIMIT ? OFFSET ?";

        try {
            con = DriverManagerConnectionPool.getConnection();
            statement = con.prepareStatement(query);
            statement.setInt(1, limit);  // Numero di post per pagina
            statement.setInt(2, offset); // L'offset in base alla pagina
